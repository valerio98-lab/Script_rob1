function [q_tau, coefficients] = cubic_compute_joint_trajectories(q_in, q_fin, T, p_dot_0, p_dot_1, link_len, direct_kinematics, print_info)
    ...
    %%Esempio di uso: 
    %[q_tau, coefficients] = cubic_compute_joint_trajectories([-1.78, 2.40],[0, 1.57],3,[-2;0], [2;2], [1,1], DK, false);
    ...
    % Velocity in t-domain
    q_dot_0 = J_initial_inv * p_dot_0;   % [rad/s]
    q_dot_T = J_initial_inv * p_dot_1;   % [rad/s]
    
    % Velocità in τ-domain (serve a cubic_poly_double_norm_compute_coeff)
    q_dot0_tau = q_dot_0 * T;    
    q_dotT_tau = q_dot_T * T;  
    ...
    for i = 1:num_joints
        ...
        [q, c] = cubic_poly_double_norm_compute_coeff( ...
                       q_in(i), ...
                       q_fin(i), ...
                       q_dot0_tau(i), ...   % ← qui
                       q_dotT_tau(i), ...   % ← e qui
